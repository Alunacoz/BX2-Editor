...
function renderPreview() {
  ...
  ctx.fillStyle = bgOpaque ? `rgb(${bgR},${bgG},${bgB})` : `rgba(${bgR},${bgG},${bgB},0.45)`
  ctx.fillRect(0, 0, W, H)
  ...
  for (const eff of state.effects) {
    ...
    const strokeW = eff.strokeWidth > 0 ? (eff.strokeWidth / 100) * actualFontSize * 2 : 0
    lines.forEach((line, li) => {
      const lineCenterY = centerY + (li - (lines.length - 1) / 2) * lineH
      if (strokeW > 0) {
        ctx.strokeStyle = eff.strokeColor || '#000000'
        ctx.lineWidth = strokeW
        ctx.lineJoin = 'round'
        ctx.shadowBlur = 0
        ctx.strokeText(line, tx, lineCenterY + baselineOffset)
        ctx.shadowBlur = Math.max(2, Math.ceil(actualFontSize / 10))
      }
...
function getEffectiveColors(frame) {
  ...
  for (const ef of state.effects) {
    ...
    if (ef.bgColor) {
      ...
      bgRgb = bgRgb ? lerpRgbArr(bgRgb, target, fade) : lerpRgbArr(hexToRgbArr('#0a0b0f'), target, fade)
...
function defaultEffectProps(type) {
  ...
  if (type === 'pathColor') return {
    pathColor: '#e05050', ballColor: '#ffffff', bgColor: '#0a0b0f', bgOpaque: false, fadeIn: 60, fadeOut: 60,
  }
...
function renderEffectProps() {
  ...
  if (ef.type === 'text') {
    typeFields = `
      ...
      <div class="prop-row">
        ...
        <input type="color" class="prop-color" id="fxStrokeColor" value="${ef.strokeColor||'#000000'}">
        <span class="prop-color-hex" id="fxStrokeColorHex"><input class="prop-hex-input" id="fxStrokeColorHexInput" value="${ef.strokeColor||'#000000'}"></span>
      </div>
      <div class="prop-row">
        <label class="prop-label">Stroke W</label>
        <input type="range" class="prop-slider" id="fxStrokeSlider" min="0" max="20" step="0.5" value="${ef.strokeWidth??0}">
        <input type="text" class="prop-input prop-num" id="fxStrokeWidth" value="${(ef.strokeWidth??0).toFixed(1)}" style="width:40px">
  ...
  if (ef.type === 'text') {
    ...
    wireColorInput('fxColor', 'fxColorHexInput', () => ef.color, v => { ef.color = v; saveLastEffectSettings(ef.type, ef) })
    ...
    wireColorInput('fxStrokeColor', 'fxStrokeColorHexInput', () => ef.strokeColor||'#000000', v => { ef.strokeColor = v; saveLastEffectSettings(ef.type, ef) })
    const stS = document.getElementById('fxStrokeSlider'), stI = document.getElementById('fxStrokeWidth')
    stS.addEventListener('input',  e => { ef.strokeWidth = parseFloat(e.target.value); stI.value = ef.strokeWidth.toFixed(1); saveLastEffectSettings(ef.type, ef) })
    stI.addEventListener('change', e => { const v = Math.max(0, Math.min(20, parseFloat(e.target.value)||0)); ef.strokeWidth=v; stS.value=v; stI.value=v.toFixed(1) })
  }
  if (ef.type === 'pathSpeed') {
    wireC('fxSpeed', e => { ef.speed = parseFloat(e.target.value) || 1.0 })
  }
  if (ef.type === 'pathColor') {
    wireColorInput('fxPathColor', 'fxPathColorHexInput', () => ef.pathColor, v => { ef.pathColor = v; saveLastEffectSettings(ef.type, ef) })
    wireColorInput('fxBallColor', 'fxBallColorHexInput', () => ef.ballColor, v => { ef.ballColor = v; saveLastEffectSettings(ef.type, ef) })
    const bgE = document.getElementById('fxBgColor')
    wireColorInput('fxBgColor', 'fxBgColorHexInput', () => ef.bgColor, v => { ef.bgColor = v; bgE.value = v; saveLastEffectSettings(ef.type, ef) })
    document.getElementById('fxBgOpaque').addEventListener('change', e => { ef.bgOpaque = e.target.checked; saveLastEffectSettings(ef.type, ef) })
  }
}
...
function wireColorInput(pickerId, hexId, getter, setter) {
  ...
  hexEl.addEventListener('keydown', e => {
    if (e.key === 'Escape') { hexEl.value = getter(); hexEl.blur(); return }
